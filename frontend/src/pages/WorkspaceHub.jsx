    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <aside className="hidden lg:flex w-64 bg-white flex-col border-r">
        ...
        <div className="p-3 border-t">
          <Link to="/dashboard" className="flex items-center gap-3 px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-lg">
            <FiChevronsLeft />
            All Workspaces
          </Link>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="hidden lg:block px-6 py-4 bg-white border-b">
          <h2 className="text-2xl font-bold">{activeNavItem?.fullLabel}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8 pb-20 lg:pb-8">
          ...
          {activeTab === "documents" && (
            selectedDocumentId ? (
              <>
                <button onClick={handleBackToDocs} className="flex items-center gap-2 mb-4 text-sm text-slate-600">
                  <FiArrowLeft /> Back to documents
                </button>
                <DocumentEditor documentId={selectedDocumentId} />
              </>
            ) : (
              <DocumentsList workspaceId={workspaceId} onDocumentSelect={setSelectedDocumentId} />
            )
          )}
          {activeTab === "members" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {workspace?.members.map((member) => (
                <Link to={`/profile/${member._id}`} key={member._id} className="bg-white p-4 border rounded-lg">
